...
export interface PlanResult {
  ...
  guardianBelief:   'rainbow' | 'heaven' | 'nature' | 'reincarnation' | 'continuing' | 'none'
}
...
const CLASSIFY_TOOL: Anthropic.Tool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      guardianBelief: {
        type: 'string',
        enum: ['rainbow', 'heaven', 'nature', 'reincarnation', 'continuing', 'none'],
        description: '보호자가 편지에서 언급한 사후 세계관. rainbow=무지개다리, heaven=천국/하늘나라, nature=별/바람/꽃/자연, reincarnation=다시 태어나/환생, continuing=아직 곁에/느껴져/함께 있어, none=언급 없음',
      },
    },
    required: ['riskLevel', 'primaryEmotion', 'guiltPresent', 'toneLevel', 'mustAddress', 'mustAvoid', 'replyStrategy', 'guardianBelief'],
  },
}
...
replyStrategy: 반드시 고통인정을 먼저, 안심은 나중 순서로 2-3문장.`
  ...
  try {
    ...
    const BELIEF_VALUES = ['rainbow','heaven','nature','reincarnation','continuing','none'] as const
    return {
      ...
      guardianBelief:   BELIEF_VALUES.includes(i.guardianBelief as PlanResult['guardianBelief'])
                          ? (i.guardianBelief as PlanResult['guardianBelief']) : 'none',
    }
  } catch (err) {
