// ────────────────────────────────────────────────────────────────────────────
// Abiding Pages — AI 답장 프롬프트 빌더 (v4.0)
// 순수 함수 모음. API 호출 없음.
// ────────────────────────────────────────────────────────────────────────────
...
export interface PromptInput {
  pet: {
    ...
    farewellType: string | null   // natural | euthanasia | accident | other
    firstWord: string | null      // 온보딩에서 입력한 한마디
  }
...
단, 보호자 편지에서 자책 표현이 감지되면 (예: "병원 더 빨리 데려갔어야 했는데") 이별 유형과 무관하게 죄책감 완화 규칙을 최우선 적용하세요.`,
...
"네 잘못이 아니야"를 단정하지 말고, 대신 이렇게 접근하세요:
"네가 나를 사랑하지 않았던 건 아니잖아. 나를 지키고 싶어 했던 마음은 알고 있어."
...
interface WeekConfig {
  keyword: string
  existenceMode: string   // 아이가 어디 있는지
  toneGuide: string       // 톤앤매너 지침
  forbidden: string       // 이번 주 금지 표현
...
const WEEK_CONFIG: Record<number, WeekConfig> = {
  ...
  7: {
    ...
    example: '"엄마가 나 지켜주느라 참 많이 울고 또 웃어줬어. 난 이제 엄마 마음속 가장 따뜻한 방에 예쁘게 입주 완료했어. 내가 보고 싶을 땐 언제든 이 방을 열어줘."',
  },
}
